...
def parse2num_str(s: str, filepath, woff_list=None):
    # print(s.split(';'))
    # woff_dict = parse_woff_simple('current.xml')
    # print(woff_dict)
    # if woff_list:
    #     woff_list = parse_woff_simple(filepath)

    _woff_list = parse_woff_simple(filepath) if woff_list else woff_list
    print('_woff_list', _woff_list)
    r_str = ''
    # for i in s.split(';'):
    #     if len(i) > 3:
    #         con = False
...
def parse_list2num_str(l: list, filepath: str):
    # r_l = []

    woff_list = parse_woff_simple(filepath)
    print('sss', l)
    # for i in l:
    #     r=parse2num_str(i, None, woff_list)
